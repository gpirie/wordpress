<?php
	...
	function fp_site_logo( $class='' ) {
		if ( has_custom_logo() ) 
		{
			$logo = wp_get_attachment_image_src( get_theme_mod( 'custom_logo' ) , 'full' );

        	?>
        		<a class="logo-link <?php echo ( $class ) ? 'logo-'. $class : ''; ?> u-block" href="<?php echo home_url();?>">
        			<img class="site-logo" src="<?php echo esc_url( $logo[0] );?>" alt="<?php echo get_bloginfo('name');?> logo" title="<?php echo get_bloginfo('name');?>">
        		</a>
        	<?php
		}
		else 
	...
	/* Add editor styles */
	function fp_add_editor_styles() {

		if( file_exists(  FP_THEME_INC . 'assets/css/editor-style.css' ) ) {
	    	add_editor_style( FP_THEME_INC . 'assets/css/editor-style.css' );
		}
	}
	add_action( 'admin_init', 'fp_add_editor_styles' );

	/* Customise logo output*/
	function fp_logo_class() {
	...
	add_filter( 'get_custom_logo', 'fp_logo_class' );


	/* Add meaningful class to menu items*/
	function fp_menu_class($classes, $item, $args) {
	...
	add_filter('nav_menu_css_class' , 'fp_menu_class' , 10 , 3);
	...
	function fp_submenu_class($menu, $args) {  

		$menu = preg_replace( '/ class="sub-menu"/','class="c-navmenu__submenu c-navmenu__submenu--'. $args->theme_location .'"', $menu );

		return $menu;  
	}  
	add_filter('wp_nav_menu','fp_submenu_class', 10, 2);

	/* In-article adverts*/
	function fp_insert_post_ads( $content ) 
	{
		if ( true === get_theme_mod( 'in_article' ) ) 
		{
			...
			if( false === empty( $paragraph_number ) ) {
				...
				if ( is_single() && ! is_admin() ) {
					return fp_insert_after_paragraph( $ad_code, $paragraph_number, $content );
				}
			}
		}
	...
	add_filter( 'the_content', 'fp_insert_post_ads' );

	function fp_insert_after_paragraph( $insertion, $paragraph_id, $content ) {
		$closing_p = '</p>';
	...
	function fp_navigation( $theme_location, $menu_toggle = false ) 
	{
		...
		{ 
		    ...
		    wp_nav_menu( array(
		        ...
		        'menu_class'     	=> 'c-navmenu__menu c-navmenu__menu--'. $theme_location .'  u-overflow',
	...
	function fp_post_length( $classes ) {

	    $length = fp_paragraph_count();
	...
	function fp_get_the_categories() {
	...
	function fp_get_acf_field( $key ) {
		$value = get_field( $key );
		return ( false === empty( $value ) ) ? $value : '';
	}

	function fp_get_acf_sub_field( $key ) {
	...
	function fp_display_notification($text, $type = 'error') {
