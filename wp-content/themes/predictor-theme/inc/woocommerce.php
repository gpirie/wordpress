<?php
	...
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'fp_single_add_to_cart_text' );
	function fp_single_add_to_cart_text() {
	        return __( ADD_TO_CART_TEXT, 'fp-theme' );
	}

	add_filter( 'woocommerce_product_add_to_cart_text', 'fp_archive_add_to_cart_text' );
	function fp_archive_add_to_cart_text() {
	    return __( ADD_TO_CART_TEXT, 'fp-theme' );
	}

	//Breadcrumbs
	add_filter( 'woocommerce_breadcrumb_defaults', 'fp_woocommerce_breadcrumbs' );
	function fp_woocommerce_breadcrumbs() {
	    return array(
            'delimiter'   => '',
	...
	//Add to Cart
	function fp_add_to_cart_message() {
	...
	add_filter( 'wc_add_to_cart_message', 'fp_add_to_cart_message' );

	//Checkout Fields
	function fp_checkout_fields_classes( $args, $key, $value ) { 
	...
	add_filter( 'woocommerce_form_field_args', 'fp_checkout_fields_classes', 10, 3 );
	
	function fp_empty_cart_message() {
	...
	add_filter( 'wc_empty_cart_message', 'fp_empty_cart_message' );

	//remove product tabs but keep description
	function fp_remove_product_tabs( $tabs ) {
		unset( $tabs['description'] );
		unset( $tabs['reviews'] );
		unset( $tabs['additional_information'] );
		return $tabs;
	}
	add_filter( 'woocommerce_product_tabs', 'fp_remove_product_tabs', 98 );
	...
	function fp_thankyou( $content ) {
	    ...
	    if ( ! $order ) {
	        ...
	        {
	            do_action( 'fp_thankyou_failed', $order );
	        } 
	        else 
	        {	           
	           do_action( 'fp_thankyou_successful', $order );
	        }
	    }
	...
	add_filter( 'the_content', 'fp_thankyou' );
	
	function fp_thankyou_failed( $order ) {
	    wc_get_template( 'woocommerce/thankyou/failed.php', array( 'order' => $order ) );
	}
	add_action( 'fp_thankyou_failed', 'fp_thankyou_failed', 10 );
	
	function fp_thankyou_header( $order ) {
	    wc_get_template( 'woocommerce/thankyou/header.php', array( 'order' => $order ) );
	}
	add_action( 'fp_thankyou_successful', 'fp_thankyou_header', 10 );
	
	function fp_thankyou_table( $order ) {
	    wc_get_template( 'woocommerce/thankyou/table.php', array( 'order' => $order ) );
	}
	add_action( 'fp_thankyou_successful', 'fp_thankyou_table', 20 );
	
	function fp_thankyou_customer_details( $order ) {
	    wc_get_template( 'woocommerce/thankyou/customer-details.php', array( 'order' => $order ) );
	}
	add_action( 'fp_thankyou_successful', 'fp_thankyou_customer_details', 30 );	

	function fp_get_recent_customer_orders( $count = 5 ) {
		...
		{
			fp_display_notification( 'You have no recent orders. Place an order today.', 'notice' );
		}
	}