...
function starter_site_logo() {
	if ( has_custom_logo() ) {
		...
		echo '<a class="site-logo" href="' . esc_url( get_home_url() ) . '"><img src="' . $image[0] . '" alt="" title="" /></a>';
	} else {
		$description = get_bloginfo( 'description', 'display' );
		$title = get_bloginfo( 'title', 'display' );

		if ( $title ) {
			echo '<a class="site-logo" href="' . esc_url( get_home_url() ) . '"><h1 class="site-title">' . $title . '</h1></a>';
		}
	}
}
...
function starter_navigation( $theme_location, $menu_toggle = false ) {
	if ( has_nav_menu( $theme_location ) ) {
		if ( true === $menu_toggle ) {
			echo '<a class="c-toggle c-toggle--' . $theme_location . ' u-block u-hidden@md u-hidden@lg u-hidden@print" href="#">Menu</a>';
		}
...
function starter_social_links( $class = '' ) {
	...
	if ( false === empty( $social_links ) ) {
		...
		$html = '<ul class="o-sociallinks u-overflow ' . $class . '">';
...
function starter_post_meta() {
	...
	echo '<div class="o-post__meta"><p>' . $post_meta . '</p></div>';
}
