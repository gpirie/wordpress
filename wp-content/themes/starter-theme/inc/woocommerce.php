 ...
 * @since    1.0.0
 */
function starter_custom_redirect_after_purchase() {
	global $wp;
...
add_action( 'template_redirect', 'starter_custom_redirect_after_purchase' );
 ...
 * @return array
 */
function starter_thankyou( $content ) {
	...
	if ( ! $order ) {
		...
		?><p><?php echo apply_filters( 'woocommerce_thankyou_order_received_text', __( 'Thank you. Your order has been received.', 'starter-theme' ), null ); ?></p><?php
	} else {
		if ( $order->has_status( 'failed' ) ) {
			do_action( 'starter_thankyou_failed', $order );
		} else {
			do_action( 'starter_thankyou_successful', $order );
		}
	}
...
add_filter( 'the_content', 'starter_thankyou' );
